"""
LightGBM BentoML service builder
Handles creation and code generation for LightGBM models
"""
...
from app.services.bentoml.builders.base import (
    BENTOML_AVAILABLE, LIGHTGBM_AVAILABLE, JOBLIB_AVAILABLE,
    bentoml, lgb, joblib, settings
)
...
class LightGBMBuilder:
    """Builder for LightGBM BentoML services"""
    ...
    async def create_service(self, model: Model, service_name: str, config: Dict[str, Any]) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            bento_model_tag_obj = await asyncio.to_thread(
                ...
                name=f"lightgbm_model_{model.id.replace('-','_')}",
                model=loaded_model,
                labels={
                    "framework": "lightgbm",
